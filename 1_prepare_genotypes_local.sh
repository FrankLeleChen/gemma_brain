...
#Merge all datasets, into chromosome sized pieces
 for chr in {1..22}
 do
  scripts/merge_datasets_local.sh -g $starting_geno_dir -p $p_loc -n $sname -o $outdir_mgeno -a yes -q $gtthresh -m $mafthresh -e $filterfile -k $keep -x $lisa
 done
